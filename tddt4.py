...
if __name__ == "__main__":
    ...
    if 1:
        ...
        plt.plot(solver.xc, U_0[1, :].T, '--', label='Initial 2')
        # plt.plot(x, uc, '--')
        plt.plot(solver.x, Ue[0, solver.gc:-(solver.gc)].T, '--', label='Euler 1')
        plt.plot(solver.x, Ue[1, solver.gc:-(solver.gc)].T, '-', label='Euler 2')
        ...
        plt.plot(solver.x, Urk3[1, solver.gc:-(solver.gc)].T, '-.', label='RK3 2')
        plt.legend()
        plt.show()