...
from clawpack.riemann.euler_with_efix_1D_constants import density, momentum, \
    energy, num_eqn
...
a = np.array([[0., 0., 0., 0., 0., 0., 0.],
              ...
              [.2429952205373960, .2429952205373960, .2429952205373960, 0., 0.,
               0., 0.],
              [.1535890676951260, .1535890676951260, .1535890676951260,
               .2384589328462900, 0., 0., 0.]])
b = np.array(
    [.206734020864804, .206734020864804, .117097251841844, .181802560120140,
     .287632146308408])
c = np.array([0., .3772689153313680, .7545378306627360, .7289856616121880,
              .6992261359316680])


def setup(use_petsc=False, iplot=False, htmlplot=False, outdir='./_output',
    ...
    state.q[energy, :] = pressure / (gamma - 1.) + 0.5 * state.q[density,
                                                         :] * velocity ** 2
